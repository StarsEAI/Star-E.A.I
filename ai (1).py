...
class Inputs(keras.layers.Layer):
    ...
    def call(self,inputs):
        ...
        # encoded_ids = tf.cast(inputs[0],dtype=tf.float32)
        encoded_ids = inputs
        # print(encoded_ids)
        # quit()
        ...
        # sequence = sequence.reshape((512))
        # padding_mask = padding_mask.reshape((512))
...
class TransformerLayer(keras.layers.Layer):
    ...
    def call(self,x):
        ...
        '''
        tr_output = []
        original = tf.constant(np.array([x[0]]))
        sequ = tf.constant(np.array([x[0]]))
        print(len(x))
        try:
            attn_mask = tf.constant(np.array([x[1]]))
        except:
            attn_mask = None
            pass
        if attn_mask != None:
            ...
            print(sequ.shape)
            mham_output = self.mham(query=sequ, value=sequ)
            print(mham_output.shape)
            sequ = sequ + self.dropout(mham_output)
            sequ = self.add([original,sequ])
            sequ = self.ln1(sequ)
            sequ1 = sequ
...
dataset = tf.data.Dataset.load("compiled_dataset")
n = 0
    ...
    if t == 0: 
        ...
        class MyModel(keras.Model):
            def __init__(self):
                ...
                self.custominputs = Inputs(max_sequence_length=512)
                self.tr_layer1 = TransformerLayer(512, 128, num_heads=6, vocab_size=30000, dropout=0.1)
                self.tr_layer = TransformerLayer(512, 128, num_heads=6, vocab_size=30000, dropout=0.1)
                self.dcwp = DCWP(1,model,512)
            def call(self,inputs):
                x = self.custominputs(inputs)
                x = self.dcwp(x)
                x = self.tr_layer1(x)
                x = self.tr_layer(x)
                x = self.tr_layer(x)
